...
class PylintRunner(LintRunner):
    ...
    regex = re.compile(r"\[(?P<type>[RCWEF])(?P<remainder>.*?)\]")
    fixup_map = {'R': 'Refactor', 'C': 'Convention',
    ...
    def run_flags(self):
        return ('-c',
                ...
                #'import sys,pylint.epylint;pylint.epylint.lint(sys.argv[1:])',
    ...
    def _replacement(self, mObj):
        "Alter to include 'Error' or 'Warning'"
        replacement = self.fixup_map[mObj.group("type")]
    ...
    def run(self, filename):
        ...
        childPath = os.path.basename(fullPath)

        while parentPath != "/" and \
            os.path.exists(os.path.join(parentPath, '__init__.py')):
                childPath = os.path.join(os.path.basename(parentPath),
                                         childPath)
                parentPath = os.path.dirname(parentPath)

        cmdline.append(childPath)

        env = dict(os.environ, **self.env)

        print  # add formatting space
        process = Popen(cmdline, stdout=PIPE, stderr=PIPE,
                        env=env, cwd=parentPath)
        p = getattr(process, self.stream)
        for line in p:
            # remove pylintrc warning
            if line.startswith("No config file found"):
                continue
            line = self.regex.sub(self._replacement, line, 1)
            print line,
        p.close()
...
def main():
    ...
    #import epdb;epdb.serve()
    config = find_config(os.path.realpath(sys.argv[1]), 'save')
